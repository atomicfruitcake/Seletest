...
public class CommonFunctions {
	...
	public static String getSettings(int entryNo) {
		try {
			...
			throw new RuntimeException("Error fetching data from settings.txt for line " + entryNo, e);
		}
	}
	...
	public static void setItemInLocalStorage(String item, String value, WebDriver driver) {
	...
	public static void clickElementByXpath(WebDriver driver, String cssSelector, String xpath) {
		waitForElement(driver, cssSelector);
	...
	public static void clickElementById(WebDriver driver, String cssSelector, String id) {
		waitForElement(driver, cssSelector);
	...
	public static void clickElementByName(WebDriver driver, String cssSelector, String name) {
		waitForElement(driver, cssSelector);
	...
	public static void clickElementByClassName(WebDriver driver, String cssSelector, String className) {
		waitForElement(driver, cssSelector);
	...
	public static void sendKeysToElement(WebDriver driver, String cssSelector, String keys) {
		waitForElement(driver, cssSelector);
	...
	public static void mouseHover(WebDriver driver, String cssSelector) throws AWTException {
		threadSleep(3);
	...
	public static void selectFromDropDown(WebDriver driver, String cssSelector, int index) {
		CommonFunctions.waitForElement(driver, cssSelector);
		WebElement dropDownListBox = driver.findElement(By.cssSelector(cssSelector));
		...
		if (getScreenshotToggle().toLowerCase() == "yesscreenshot") {
			...
			try {
				FileUtils.copyFile(screenshot,
						new File("Screenshots\\" + name + "_" + fullFormat.format(dDate) + ".png"));
			}
	...
	public static void verifyExists(WebDriver driver, String cssSelector) {
		...
		waitForElement(driver, cssSelector);
		Assert.assertNotEquals(driver.findElement(By.cssSelector(cssSelector)), null);
	}

	// Verify the text on a given web element
	public static void verifyText(WebDriver driver, String cssSelector, String text) {
		LOGGER.info("Asserting " + cssSelector + " contains the text " + text);
		waitForElement(driver, cssSelector);
		Assert.assertEquals(driver.findElement(By.cssSelector(cssSelector)).getText(), null);
	}
	...
	public static void verifyDoesNotExist(WebDriver driver, String cssSelector) {
		...
		Assert.assertEquals(driver.findElement(By.cssSelector(cssSelector)), null);
	}
	...
	public static void buildTestngXml() throws IOException {
		...
		Element suite = document.addElement("suite").addAttribute("name", "testSuite")
				.addAttribute("preserve-order", "true").addAttribute("parallel", "false").addAttribute("verbose", "10");
	...
	public static void updateSlackTestBot(String content) {
		...
		try {
			new Slack(SLACK_WEBHOOK).icon(":smile:").sendToChannel("").displayName("testbot")
	 ...
	 */
	public static void runTerminalCommand(String curlCommand) throws InterruptedException {
	...
	public static void tunnelIntoServer(String shellCommand, int port) throws JSchException, IOException {
	...
	public static void writeToTextFile(String text, String filename) {
		...
		try {
			...
			if (!file.exists()) {
				LOGGER.info(filename + " does not exist. Building file " + filename);
				file.createNewFile();
	...
	public static void scanTextFileForMatch(File file, String matchText) {
		LOGGER.info("Scanning " + file.getName() + " to check if is contains: " + matchText);
	...
	public static void verifyHttpResponseCode(String url, int responseCodeExpected) {
		LOGGER.info("Asserting reponse code for " + url + " is equal to " + responseCodeExpected);
	...
	public static void updateSlackAfterSuite(ITestContext testContext) throws IOException {
		if (getUpdateSlack().equals("yesUpdateSlack")) {
			...
			updateSlackTestBot(getTestSuite() + " has been run on " + getEnvironment() + "\n" + "\n Passed: "
